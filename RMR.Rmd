...
lm_lasso <- lm(
  ...
  data = recipe_list$origScale |> step_select(all_of(c(all_predictors(), all_outcomes()))) |> prep() |> bake(NULL),
  x = TRUE,
  y = TRUE
)
lm_lasso_orig <- lm_lasso
lm_lasso$coefficients <- selected_coefs$estimate
names(lm_lasso$coefficients) <- names(lm_lasso_orig$coefficients)
lm_lasso$fitted.values <- as.numeric(lm_lasso$x %*% lm_lasso$coefficients)
names(lm_lasso$fitted.values) <- rownames(lm_lasso$x)
lm_lasso$residuals <- lm_lasso$y - lm_lasso$fitted.values

# Recompute residual standard error and R-squared
rss <- sum(lm_lasso$residuals^2)  # Residual Sum of Squares
tss <- sum((lm_lasso$y - mean(lm_lasso$y))^2)  # Total Sum of Squares
mss <- sum((lm_lasso$fitted.values - mean(lm_lasso$fitted.values))^2)
n <- nrow(lm_lasso$x)  # Number of observations
p <- length(lm_lasso$coefficients)  # Number of predictors including intercept
lm_lasso$df.residual <- n - p  # Degrees of freedom for residuals
lm_lasso$sigma <- sqrt(rss / lm_lasso$df.residual)  # Residual standard error
lm_lasso$r.squared <- 1 - (rss / tss)  # R-squared
lm_lasso$adj.r.squared <- 1 - (1 - lm_lasso$r.squared) * ((n - 1) / lm_lasso$df.residual)  # Adjusted R-squared
# Update effects
lm_lasso$effects <- qr.qty(qr(lm_lasso$x), lm_lasso$y)

# Update QR decomposition
lm_lasso$qr <- qr(lm_lasso$x)

# Validate assign field
lm_lasso$assign <- attr(lm_lasso$terms, "assign")
...
linear_imp <- relaimpo::calc.relimp(lm_lasso, type = feature_imps)
...
plot_bland_altman(nuremberg_preds[model %in% interesting_models], nuremberg_metrics, main_metric)
```
...
munich_data[, SEX := factor(SEX, levels = c('männlich', 'weiblich'))]
...
plot_bland_altman(munich_preds[model %in% interesting_models], munich_metrics[model %in% interesting_models], main_metric)
```
