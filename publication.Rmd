...
compute_my_metrics <- function(preds, my_metrics, metrics_to_keep){
...
require(tidytext)
summary_metrics <- data.table::melt(metric_dt, id.vars = c('model', 'Site', 'metrics_string'), variable.name = 'metric_name', value.name = 'metric', variable.factor = FALSE)
...
score_summary <- ggplot(summary_metrics[model %in% interesting_modelmap & 
                       metric_name %in% interesting_metricmap &
                       Site %in% c("Freising", "Nuremberg", "Kiel")],
      aes(
          x = tidytext::reorder_within(model, metric, Site),
          y = metric,
          fill = `Model Type`
        )) +
        ...
        geom_text(aes(label = round(metric, rounding_by[metric_name]), y = 0), hjust = -.15, angle = 90) +
        facet_grid(metric_name ~ Site, scales = "free") +
        labs(x = NULL, y = NULL) + 
        tidytext::scale_x_reordered() + 
        theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "top")
ggsave(filename = file.path(fig_dir, "score_summary.pdf"), plot = score_summary, width = 7, height = 4)

summary_metrics[Site == "KielWoTemp", Site := "Kiel Without Temperature"]
score_summary_kiel <- ggplot(summary_metrics[model %in% interesting_modelmap[!interesting_modelmap %in% established_models] & 
                       metric_name %in% interesting_metricmap &
    ...
    FUN = function(sites, models, fig.sizes) {
      ...
      ba <-
        plot_bland_altman(
          pred_dt[model %in% models &
                    Site %in% sites],
          metric_dt = metric_dt[model %in% models &
                                  Site %in% sites],
          main_metric = interesting_metricmap[main_metric],
          facet_col = ifelse(length(sites) > 1, "Site", ""),
          facet_row = ifelse(length(models) > 1, "model", ""),
...
pred_obs_score_summary <- ggarrange(pred_obs_freising, score_summary, widths = c(4,7), labels = c("A", "B"))
pred_obs_score_summary
ggsave(file = file.path(fig_dir, "pred_obs_score_summary.pdf"), plot = pred_obs_score_summary, width = 11, height = 4)
```
...
clust <- hclust(dist_obj, method = "ward.D2")
clust <-
